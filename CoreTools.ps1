...
foreach ($folder in $phoenixFolders) {
    if (Test-Path $folder) {
        Write-Host "-- Removing PhoenixOS folder: $(Split-Path $folder -Leaf)" -ForegroundColor Gray
...
foreach ($uPath in $userPaths) {
    ...
    # Metrics and Safe Browsing set to 0 (Disabled) but toggleable
    Set-ItemProperty -Path $uPath -Name "MetricsReportingEnabled" -Value 0 -Type DWord -ErrorAction SilentlyContinue
...
if ([console]::KeyAvailable) {
    $choiceDNS = [console]::ReadKey($true).KeyChar
    Write-Host ""
} else {
...
if ($choiceDNS -eq 'y' -or $choiceDNS -eq 'Y') {
    ...
    # ONLY applying to Brave HKCU to keep the button clickable (Blue)
...
Write-Host "`[MODULE] Checking Icaros Thumbnailer (Images & Videos) via GitHub API..." -ForegroundColor Cyan
